...
**Issue:** Microsoft's tenant-level MFA enforcement triggered on the break-glass account even though it was explicitly excluded from the custom Conditional Access Policy.
**Root cause:** Microsoft enforces a separate tenant-wide MFA requirement for administrative portal access that operates independently of custom Conditional Access exclusions.
**Fix:** Documented as an architectural constraint rather than a configuration error. In production this is resolved by binding the break-glass account to a FIDO2 hardware key rather than a personal device.
**Lesson:** Custom Conditional Access exclusions do not override all Microsoft-enforced security defaults. Always test break-glass accounts after policy changes to confirm emergency access still works.
...
**Issue:** PIM activation did not always trigger a fresh MFA prompt during testing.
**Root cause:** Microsoft's tenant-level authentication session can satisfy PIM's MFA requirement if the user already authenticated with MFA when signing in. PIM does not always force a separate re-authentication.
**Fix:** Documented as expected behavior rather than a configuration error.
**Lesson:** Layered authentication controls interact in non-obvious ways. Understanding how existing session tokens satisfy downstream MFA requirements is important when designing privileged access workflows.
...
**Troubleshooting & Lessons Learned — Lab 3A:**

**Issue:** Access package wizard showed no available groups even though groups existed in the directory.
**Root cause:** Wrong catalog selected on the Basics tab. The wizard only shows resources belonging to the selected catalog — if the catalog is set to "General" instead of the custom catalog, none of the custom resources appear.
**Fix:** Returned to Basics tab, selected Marketing Department Resources catalog, and groups appeared immediately.
**Lesson:** Always confirm the catalog selection on the Basics tab before proceeding to Resource roles. This dependency is not clearly surfaced in the UI and is a common point of confusion.

**Troubleshooting & Lessons Learned — Lab 3B:**

**Issue:** End-to-end SSO test returned HTTP 400 GENERAL_NONSUCCESS error.
**Root cause:** Okta's default attribute parser cannot read Entra ID's XML schema URI format for user attributes (e.g. `http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname`). The JIT provisioning engine failed because it couldn't parse the incoming claims.
**Fix:** Built custom attributes in the Okta Profile Editor mapping the full XML URI strings to simple profile fields, then mapped those custom attributes to Okta's standard user profile fields.
**Lesson:** SAML federation failures are rarely obvious from the error message alone. Reading the Okta System Log and tracing the failure chain (Unknown Profile Attribute → user creation failure → JIT failure) is the correct diagnostic approach.
...
**Issue:** After submitting approve and deny decisions, the access review became stuck in a processing state and the Stop button was not available.
**Root cause:** The review was created during a previous session and had entered a stuck state, likely due to a tenant subscription transition that occurred mid-configuration.
**Fix:** Deleted the stuck review and recreated it from scratch. The Stop button appeared immediately after submitting decisions on the new review.
**Lesson:** Governance workflows can enter stuck states after tenant configuration changes. Recreating rather than debugging a stuck campaign is the faster operational resolution.
...
**Issue:** Conditional Access for agent identities triggered MFA on the break-glass account during testing even though it was excluded from the policy.
**Root cause:** Same tenant-level MFA enforcement behavior documented in Lab 1 — Microsoft's default admin portal security requirements operate independently of custom Conditional Access exclusions.
**Fix:** Documented as a licensing and architectural constraint. Full agent-risk Conditional Access enforcement will require Microsoft Agent 365 licensing as enforcement rolls out.
**Lesson:** Preview features in enterprise identity platforms often have licensing and enforcement timelines that differ from their documentation. Documenting the current behavior alongside the expected future state is part of responsible production planning.
